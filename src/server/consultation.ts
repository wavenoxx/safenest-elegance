...
// Rate limiting state: in-memory map per IP (10 requests per 10 minutes)
...
export const submitConsultationServerFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<ConsultationResponse> => {
    ...
    try {
      // 3. Database Insertion via Supabase Admin Client (Service Role)
      ...
      // 4. Trigger Server-Side WhatsApp Owner Notification (async non-blocking)
      notifyOwnerWhatsApp({
        leadId,
